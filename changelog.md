...
|1.0.0|`51dcde7ca9af3cf9636efee239c4a1bd660e0d8b`|Complete overhaul of the library. Replaced the Chat Completions-based architecture (Azure OpenAI, Ollama) with the **OpenAI Responses API** via `TimHanewich.Foundry`. New simplified `Agent` API with `PromptAsync(string)` that returns a string. Added `ExecutableFunction` abstract class for tools with automatic execution loop. Added `AgentAsTool` for multi-agent orchestration. Added recursive token tracking, `ReasoningEffortLevel`, `VerbosityLevel`, and `WebSearchEnabled` settings.|
|1.1.0|`40521acc36450f9cfa84a3bb94f62a24e09ea0d5`||